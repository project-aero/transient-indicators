...
rdf <- odf %>% 
  ...
  mutate(intercept = purrr::map_dbl(lmod, intercept))
...
## # A tibble: 2 x 6
## # Groups:   outbreak [2]
##   outbreak           data lmod    stat maxamp intercept
##   <chr>    <list<df[,7]>> <list> <dbl>  <dbl>     <dbl>
## 1 mn2017         [38 × 7] <lm>    1.43     65    -10.3 
## 2 ca2014         [39 × 7] <lm>    2.76    110      7.33
```
...
pmeasles <-
  ...
  geom_hline(yintercept = rdf$maxamp, color = "grey", size = 1 / 5) +
  geom_abline(slope = rdf$stat, intercept = rdf$intercept, color = "grey", size = 1 / 5) + 
  geom_step() +
